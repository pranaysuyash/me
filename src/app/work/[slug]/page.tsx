...
export async function generateMetadata({ params }: WorkDetailPageProps): Promise<Metadata> {
  ...
  const preview = audited?.visualEvidence[0];
      ...
      <article>
        ...
        <section className="py-12 md:py-16">
          <div className="container mx-auto max-w-[1120px] px-4 md:px-6 lg:px-8">
            <p className="mb-5 text-xs font-semibold uppercase tracking-[0.18em] text-primary">Visual evidence</p>
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              {project.visualEvidence.map((visual, index) => (
                <figure key={visual.src} className={index === 0 ? "sm:col-span-2" : ""}>
                  <div className="relative aspect-[16/9] overflow-hidden rounded-xl border bg-[#0d1718]">
                    <Image
                      src={visual.src}
                      alt={visual.alt}
                      fill
