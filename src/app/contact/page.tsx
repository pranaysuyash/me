...
export default function ContactPage() {
  ...
  const selectMode = (nextMode: ContactMode) => {
    ...
    setFormData((previous) => ({
      ...previous,
      source: `${sourceContext}:${nextMode}`,
      mode: nextMode,
      ...(nextMode === "role" ? { timeline: "", budget: "" } : {}),
    }));
      ...
      <section className="border-b bg-[#0d1718] text-white">
        <div className="container mx-auto max-w-[1280px] px-4 py-16 md:px-6 md:py-24 lg:px-8">
          ...
          <h1 className="mt-5 max-w-5xl text-4xl font-bold tracking-[-0.045em] sm:text-5xl md:text-6xl">
            {roleMode
              ? "Share the role, the team, and where sustained ownership matters."
              : "Describe the work as it exists today."}
          </h1>
          <p className="mt-6 max-w-4xl text-base leading-8 text-white/70 md:text-lg">
            {roleMode
              ? "The useful first message includes the mandate, reporting context, product stage, team shape, and why a product leader who stays close to implementation would be valuable."
